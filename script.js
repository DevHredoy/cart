...
let modifiedFruitList = uniqueFruits.map((fruit) => {
  return { ...fruit, amount: 0, roughcount: 10 };
});

let myfruits = document.querySelector("#body-contents");
...
function displayCart() {
  myfruits.innerHTML = modifiedFruitList
    .map((item, i) => {
      let { image, title, price, roughcount } = item;
...
function plusMod(indexofFruit_mod) {
  ...
  //here this display cart is not called after one increase of roughcount
  displayCart();
}
